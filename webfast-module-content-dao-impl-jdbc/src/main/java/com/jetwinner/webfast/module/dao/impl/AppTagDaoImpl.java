...
import com.jetwinner.webfast.module.dao.AppTagDao;
...
public class AppTagDaoImpl extends FastJdbcDaoSupport implements AppTagDao {
    ...
    public Map<String, Object> addTag(Map<String, Object> tagMap) {
        Number idNumber = insertMapReturnKey("app_tag", tagMap);
        return getJdbcTemplate().queryForList("SELECT * FROM app_tag WHERE id = ?", idNumber.intValue())
                .stream().findFirst().orElse(null);
    }

    @Override
    public Map<String, Object> getTagByName(String name) {
        return getJdbcTemplate().queryForList("SELECT * FROM app_tag WHERE name = ? LIMIT 1", name)
                .stream().findFirst().orElse(null);
    }
    ...
    public List<Map<String, Object>> getTagByLikeName(String partOfName) {
        ...
        return getJdbcTemplate().queryForList("SELECT * FROM app_tag WHERE name LIKE ?", name);
    }
}
